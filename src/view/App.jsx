  ...
  return (
    <Router>
      <Switch>
        ...
        <Route path="/reports">{token ? <ReportsPage token={token} handleToken={handleToken} /> : <Redirect to="/login" />}</Route>

        <Route path="/createreport">
          {token ? <CreateReportPage token={token} handleToken={handleToken} /> : <Redirect to="/login" />}
        </Route>
      </Switch>
    </Router>
  );
